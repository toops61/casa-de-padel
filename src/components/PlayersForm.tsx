...
export default function PlayersForm() {
    ...
    const addNewPlayer = (e:ChangeEvent<HTMLFormElement>) => {
        e.preventDefault();
        const newSelected = e.target.newPlayer.value.trim().replace(/ +(?= )/g,'');
        const newPlayer = {name:newSelected,id:nanoid()};
        if (newPlayer.name && !players.some(player => player.name.toLowerCase() === newPlayer.name.toLowerCase())) {
            addPlayer(newPlayer);
            updateInitial([...initialPlayers,newPlayer]);
            showModal('Nouveau joueur créé','');
        } else {
            const response = newPlayer.name ? `Il existe déjà un joueur ${newPlayer.name}` : 'Il faut donner un nom au joueur...'
            showModal(response);
        }
        ...
        <div className="inputs-container">
            <div className="players-input">
                ...
                <select name="players" onChange={selectPlayer}>
                    ...
                    {playersDB.filter(e => !players.some(el => el.name.toLowerCase() === e.name.toLowerCase()) && !playersPlaced.some(el => el === e.id)).map(player => <option value={player.name} key={player.id}>{player.name}</option>)}
                </select>
            </div>
